...
const Input = ({
  ...
  return (
    <div
      className={`group relative my-3 mb-6 w-full border-[1px] border-gray-200 focus-within:border-primary ${outerClass} ${
        isValid === false &&
    ...
    >
      {label && (
        <label
         ...
         font-light capitalize text-gray-200 group-focus-within:text-primary ${
      ...
      <input
        ...
        ref={inputRef}
        className={`h-max w-full p-2 sm:p-3 text-black placeholder:font-gray-100 placeholder:text-gray-400 focus:outline-none 
        focus:placeholder:text-primary  ${
          isValid === false &&
          "focus:placeholder:text-red-500 placeholder:text-red-500"
        } ${className} ${label && "!my-0 !py-0 !pb-2 placeholder:opacity-0"}`}
        onKeyDown={onEnterPressHandler}
      />
      {!isValid && error && (
        <p
          className={`absolute -bottom-4 left-3 text-xs font-black text-red-500`}
        >
          {error}
        </p>
      )}
